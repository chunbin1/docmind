...
import { initChatTables, createConversation, appendMessage, getMessages } from './chatStore.js'
...
test('客户端断开（send 抛错）仍把答案生成完并落库 done', async () => {
  const db = setup()
  const c = createConversation('u1').id
  const { id } = appendMessage('u1', c, { role: 'assistant', content: '', status: 'generating' })
  let calls = 0
...
test('流抛错时落库 status=error 并向上抛', async () => {
  ...
  assert.equal(getMessages(c)[0].status, 'error')
  db.close()
})
